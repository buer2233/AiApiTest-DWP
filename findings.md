...
| Docker/Jenkins/scripts 注释补齐不改变执行行为 | 本次只为 Jenkinsfile、Groovy Pipeline、Dockerfile、部署脚本和 Jenkins 静态测试添加中文说明注释；Jenkins 静态测试 15 passed，Compose 配置校验通过 |
...
| Bash 语法检查在本机打印 WSL 环境提示 | `bash -n scripts/deploy-docker.sh` 退出码为 0，提示来自本机 WSL 环境，不是脚本语法错误 |
