...
- 当前会话提供的可用 Skills 清单是唯一可调用事实来源；磁盘存在某个 `SKILL.md` 不代表当前可调用。
- 开始任务时按任务匹配并选择能直接降低风险的最小 Skill 集合。主人明确点名的 Skill 如在当前会话可用，应按其说明执行。
- Skill 不是质量门禁，也不定义开发阶段。需求、测试、TDD、审查、证据、安全和容器化要求均以本文件及子目录规则为准。
- Skill 不可用、执行失败或名称发生变化时，说明对当前任务的实际影响，并按本文件中的行为协议继续；不得跳过任何适用阶段或质量门禁。
- 当前项目常见的可选增强包括：Python 测试可按需使用 `python-testing`，UI/UX 与设计系统工作可按需使用 `ui-ux-pro-max`、`design-system`，架构图可按需使用 `drawio-skill`，位图视觉资产可按需使用 `imagegen`。每次使用前仍须以当前会话清单复核。

## 工程执行协议（不依赖 Skill）

### 方案、计划与中断恢复

- 新功能、架构或行为变更在实现前必须明确目标、范围、假设和不做事项；关键决策存在多种可行方案时，至少比较方案、取舍和风险，不得静默选择。
- 规格未覆盖且会实质改变功能、权限、数据、安全、对外协议或验收口径的决策必须进入需求 §0 并熔断询问主人；普通实现细节按已冻结规格和现有代码模式判断。
- 复杂任务必须维护可追溯的执行计划并逐项更新状态。计划可写入当前阶段目录的任务文件或由会话计划工具维护，不强制为简单任务创建固定三件套文档。
- 中断恢复时先读取当前需求产物、任务计划、`git status`、当前 diff、最近验证证据和未完成门禁，再从最后一个未完成步骤继续，禁止把仓库当作无历史项目重做。

### TDD 协议

- 任何代码或可执行脚本的行为变更都必须遵循 `RED -> GREEN -> REFACTOR`：先写能表达目标行为的最小可执行测试，实际运行并确认失败原因是目标行为尚未实现，再做最小实现使目标测试通过，最后重构并运行相关回归。
- Bug 修复必须先用自动化测试稳定复现缺陷；如果初次失败来自环境、依赖或测试自身错误，该结果不算有效 RED，应先消除干扰后重新验证。
- GREEN 必须来自产品实现满足已冻结规格，不得通过删除或弱化断言、改写正确预期、跳过测试、扩大 mock 范围或只运行避开失败的子集获得。
- 文档、配置或流程治理变更优先使用静态检查、契约测试或最小可观察验证执行同等循环；纯说明性修改没有合理可执行 RED 时，应记录原因并完成针对性回归，不编造无意义测试。

### 根因排查协议

- 遇到失败或异常时按“稳定复现 -> 收集证据 -> 缩小范围 -> 建立并验证根因假设 -> 最小修复 -> 相关回归”推进。
- 修复前必须区分产品缺陷、测试缺陷、环境问题和规格冲突；不得在未验证根因时连续叠加试探性修改。
- 当前会话如果提供与问题匹配的调试 Skill，可以辅助取证，但不能替代上述协议。

### 独立审查协议

- 后端、前端及其他高风险实现完成后，必须由未参与该实现的独立 review subagent 审查；实现者不得审查自己的改动。
- reviewer 必须依据冻结需求、功能测试用例、API/UI 契约、代码 diff 和测试证据，按严重度输出问题，并提供文件和行号、风险、证据及建议；先报告问题，再给摘要。
- 主 agent 负责逐条验证审查意见，确认成立后修复并回归，再交由同一名独立 reviewer 复审。阻断问题清零前不得进入验收。
...
| M / L | 跨模块、新模块、新增或变更数据表、影响权限或报告协议的改动 | 完整 loop；每个阶段都要产出适用成果或记录 `N/A + 理由` |

裁剪规则：

- 只能裁剪产物阶段，不能裁剪质量门禁：需求澄清冻结、架构影响评估、API 契约冻结、容器化兼容检查在任何档位都必须执行；不涉及 API、页面或容器时应记录 `N/A + 理由`，不能直接略过检查。
- 档位不确定时就高不就低，按更完整的流程推进。
- XS、S 档位的裁剪决定必须可追溯，在提交备注或需求资料中写明定级和裁剪原因。M/L 的 `N/A` 只表示阶段经评估不适用，不等于省略阶段。
...
- 要求：
  ...
  - 不涉及新增页面或实质视觉/交互变更时，输出同名 UI 适用性说明即可，不强制生成无业务意义的原型图。
...
- 要求：
  - 必须依据需求文档和详细功能测试用例开发。
  - 先编写并实际运行后端 pytest/pytest-django 接口测试，确认有效 RED 后再开发接口，再回归测试。
  - 严格遵循本文件的 TDD 协议：`RED -> GREEN -> REFACTOR`。
...
- 要求：
  - UI 阶段适用时，必须依据同名 UI 原型图、后端接口、需求文档和功能测试用例开发。
  - UI 阶段评估为 `N/A` 时，必须依据同名 UI 适用性说明、需求文档、功能测试用例和适用的 API 契约开发；涉及既有页面时还要引用现有视觉基线。
  - 先把自然语言 UI 场景转成可执行 Playwright 测试并实际运行，确认有效 RED 后再开发前端页面，再回归测试。
  - 仍按本文件的 TDD 协议推进；复杂组件或组合式函数使用 Vitest + Vue Test Utils 补足更快的测试层。
...
- 独立对抗审查：后端、前端开发完成后，按本文件“独立审查协议”完成审查、修复和复审闭环。

允许的并行方式：

- 需求说明书完成并通过架构影响评估后，可以并行启动“功能测试用例设计”和“UI 原型设计”。
- UI 阶段适用时，UI 原型可以先依据需求说明书输出第一版，但必须在功能测试用例完成后进行覆盖校准。
- 后端开发必须等待功能测试用例和 API 契约冻结，不因 UI 原型提前完成而提前编码。
- UI 阶段适用时，前端开发必须等待 UI 原型、功能测试用例、后端 API 契约和 UI 区域语义拆解 / 前端实现范围映射完成；UI 阶段评估为 `N/A` 时，必须等待同名 UI 适用性说明和其他适用前置产物完成。
...
- 自动衔接：需求冻结后，功能测试用例、UI 原型或适用性说明、后端 TDD、前端 TDD 按依赖自动接力，无需逐阶段等主人点头。
