...
- 固定环境 Job 名在代码中固化为 `AiApiTest-DWP-Platform-Bootstrap`，不允许通过 `.env` 改名；它由本地 Compose Jenkins 启动时通过版本化 init Groovy 幂等创建或修复，固定加载 `jenkins/Jenkinsfile.platform-bootstrap`，不得手工创建另一条旁路 Job。
- 该 Job 固定经过八阶段：`Checkout/Workspace`、`Bootstrap Preflight`、`Dependency Assurance`、`Schema & Initial Data`、`Deploy`、`Health`、`Tests`、`Archive & Summary`。schema 阶段仅能通过 profile `bootstrap` 的一次性 `backend-bootstrap` 服务依序执行 `migrate --noinput`、`seed_environment --reconcile`、`sync_modules --reconcile`、`init_admin --bootstrap-only`。Docker 在其中是应用容器的运行载体，不是绕过 Job 的第二条环境入口。
...
- 除固定 Job 的一次性 `backend-bootstrap` 服务外，AI、宿主机、常驻服务、readiness 均禁止执行 migration 或初始化管理员。禁止 `down -v`、volume 删除、`chmod 666 /var/run/docker.sock`、`collectstatic`、自动 rollback 或输出真实凭据。环境失败必须阅读 Jenkins 结构化诊断，引导主人修复后重新构建，不能旁路处理。
