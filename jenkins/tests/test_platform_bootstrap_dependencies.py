...
def test_api_runner_hash_ignores_jenkins_allure_runtime_output(tmp_path):
    make_workspace(tmp_path)
    domain = next(item for item in default_domain_specs(tmp_path) if item.name == "api-runner")
    before = compute_domain_hashes(domain)
    report = tmp_path / "allure-report" / "index.html"
    report.parent.mkdir(parents=True)
    report.write_text("generated by Jenkins Allure plugin", encoding="utf-8")

    assert compute_domain_hashes(domain) == before
