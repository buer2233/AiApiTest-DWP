...
- 开发前必须存在同一需求命名的需求说明书、功能测试用例和适用的后端 API 契约或 `N/A + 理由` 记录。
- UI 阶段适用时，开发前必须存在同名 UI 原型；如果原型与测试用例并行产出，还必须确认已依据完整功能测试用例完成覆盖校准。
- UI 阶段评估为 `N/A` 时，必须存在同名 UI 适用性说明；涉及既有页面时说明中必须引用既有视觉基线，无需伪造新原型。
- UI 阶段适用且原型或主人标注图是复合图时，开发前必须确认 `project-info/UI/` 已输出区域语义拆解表和前端实现映射表，明确哪些区域是当前页面、点击后路由、弹窗、登录后页面、设计说明层或不实现内容。
- 开发前必须确认容器化兼容检查已完成，前端 API 地址、报告入口、静态资源路径和环境变量不能绑定个人本机配置。
- 开发顺序必须是：先把自然语言 UI 场景写成可执行 Playwright 测试，实际运行并确认失败由目标行为缺失导致，再开发页面和组件、使目标测试 GREEN、重构并回归，遵循 `RED -> GREEN -> REFACTOR`。
- 组件内部逻辑复杂时，应补充 Vitest + Vue Test Utils 单元测试。
- 阶段完成必须留存 Playwright 运行结果与关键页面截图证据，并补全 `需求名-可追溯矩阵.md` 的前端实现列。
- 页面实现完成后必须按根规则经过独立 review subagent 对抗审查；实现者不得自审，问题修复并由同一 reviewer 复审、阻断问题清零后，由主 agent 聚合 `需求名-验收包.md` 交主人终审。自主流水线整体执行方式见 `docs/自主开发流水线.md`。

## 前端工程方法与 Skill 选择

- Vue、路由、状态、请求、测试和设计质量由本文件直接约束，不依赖特定 Skill 才执行。
- 当前会话可用且任务匹配时，可使用 `ui-ux-pro-max` 辅助 UX 决策、`design-system` 辅助 token/组件规范、`imagegen` 辅助位图视觉产物；它们不能替代冻结原型、区域映射、可访问性或 Playwright 验收。
- 页面异常按根目录根因排查协议处理，先稳定复现并区分组件、路由、状态、缓存、请求、样式和环境问题，再做最小修复与相关回归。
...
- 新增组件默认使用 Composition API 与 `<script setup lang="ts">`；props、emits、route 参数和 API model 必须有明确 TypeScript 类型，复杂组件使用 typed props / emits，避免无边界的 `any`。
- 状态按生命周期放置：组件临时交互留在局部状态，跨页面客户端状态进入 Pinia，服务端数据、缓存、刷新和失效交给 TanStack Query；禁止把同一服务端状态重复维护在多个层。
- Vue Router 必须处理路由鉴权、动态参数校验、无权限和资源不存在状态；导航守卫不得代替后端权限校验。
- Axios/TanStack Query 请求要处理超时、取消、竞态、重试边界和 mutation 后缓存失效；卸载页面后不得让过期响应覆盖新状态。
- 组件应保持单一职责；复用只有在消除真实重复或隔离复杂性时才抽象，不为一次性页面制造通用框架。
- 可访问性至少覆盖语义化 HTML、键盘操作、可见焦点、表单 label 与错误关联、对话框焦点管理、色彩对比和非纯颜色状态表达。
- 测试分层：Vitest + Vue Test Utils 覆盖组件分支、组合式函数和状态逻辑，Playwright 覆盖关键用户路径、路由、真实集成状态及“应出现/不应同屏出现”；测试不得只断言实现细节。
...
- 新增页面或实质修改视觉、信息架构和交互路径时，必须先核对同名 UI 原型中的高保真视觉产物；既有页面的局部文案、校验或非视觉修复可引用现有基线并记录 UI 阶段 `N/A + 理由`。
...
- 新增页面或实质视觉/交互变更必须先对齐 `project-info/UI/` 中同名 UI 原型的风格说明；如果实现阶段调整视觉规范，应先回写 UI 原型和交互说明，并说明是否影响 `DESIGN-claude.md` 基线。
...
UI 阶段适用时，进入编码前必须基于 UI 原型补齐一份简短映射，可写入前端实现说明、RTM 或验收包。UI 阶段评估为 `N/A` 时不创建伪映射，只引用同名 UI 适用性说明：
...
- 不写死个人宿主机地址、`localhost` API 地址或不可迁移的报告文件路径；本地 Vite/Playwright 入口由平台绑定主机映射出的可连接回环地址和服务端口派生，部署验收 URL 由平台公开主机/协议/端口派生，浏览器 API 使用代码化相对路径，Docker 内代理使用固定 Compose 服务名。
