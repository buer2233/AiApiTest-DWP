    ...
    def test_login_and_get_os_info(self, base_url):
        ...
        with allure.step("2.提交明文账号密码登录"):
            login_response = api.check_login(
                loginid=self.account["user_name"],
                userpassword=self.account["password"],
            )
            assert login_response.get("msgcode") == "0", f"登录业务码异常:{LoginAPI.safe_login_fields(login_response)}"
            assert login_response.get("loginstatus") == "true", f"登录状态异常:{LoginAPI.safe_login_fields(login_response)}"
            assert login_response.get("userid") == 1, f"登录用户异常:{LoginAPI.safe_login_fields(login_response)}"
        ...
        with allure.step("4.检查登录密码策略"):
            weak_password_response = api.is_weak_password(self.account["password"])
