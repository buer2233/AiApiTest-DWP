# -*- coding: utf-8 -*-
"""静态校验 functional-to-api-test 的触发集和工作流契约集。

用法：
    python .workbuddy/skills/functional-to-api-test/evals/validate_eval_set.py
"""
...
def _validate_case_list(
    ...
    if payload.get("skill_name") != "functional-to-api-test":
        errors.append(f"{file_name} 的 skill_name 必须为 functional-to-api-test")
...
def _validate_trigger_cases(cases: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    for index, case in enumerate(cases):
        ...
        if case["should_trigger"]:
            ...
            negative_count += 1
    if positive_count < 8:
        errors.append("触发评估集至少需要 8 条正例")
    if negative_count < 5:
        errors.append("触发评估集至少需要 5 条负例")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="校验 functional-to-api-test 评估资产。")
    parser.add_argument(
        "--eval-dir",
        type=Path,
    ...
    if errors:
        for error in errors:
            print(f"错误：{error}")
        return 1
    print("functional-to-api-test 评估集校验通过")
    return 0
