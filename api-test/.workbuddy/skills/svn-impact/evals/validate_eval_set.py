...
def _validate_case_list(
    ...
    if payload.get("skill_name") != "svn-impact":
        errors.append(f"{file_name} 的 skill_name 必须为 svn-impact")
...
def _validate_trigger_cases(cases: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    if positive_count < 11:
        errors.append("触发评估集至少需要 11 条正例")
    if negative_count < 7:
        errors.append("触发评估集至少需要 7 条负例")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="校验 svn-impact 评估资产。")
    parser.add_argument(
        "--eval-dir",
        type=Path,
