...
| 6 | 生成草稿 | `runtime/capture_selection.md` |
...
flowchart TD
    ...
    Parse --> Strip[去除硬编码 Cookie/token<br/>timestamp / Referer / UA]
...
| `-H "Cookie: xxx"` | 删除硬编码，改用 `config.default_cookies` 或 fixture 注入 |
...
- 本通用版本只维护 Mermaid 源文件，不要求导出流程图片。

---

## 附录 A、PreToolUse Hook 触发时序图

描述 AI 调用 `Skill({skill: "api-test-common"})` 时，Claude Code 如何同步拦截、spawn `preflight_hook.py`、并把 `preflight_check.py` 的结果注入 AI 上下文的完整链路。
...
sequenceDiagram
    ...
    AI->>CC: Skill({skill: "api-test-common"})
...
- **二次过滤放在脚本里**：`matcher: "Skill"` 在 settings 层只能按工具名匹配，无法区分具体 skill 名；脚本内 `skill_name == "api-test-common"` 这层过滤是必须的，否则任何 Skill 调用都会触发 preflight。
