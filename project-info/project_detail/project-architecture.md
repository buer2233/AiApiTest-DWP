...
每个需求必须按固定 loop 推进，但固定 loop 表示产物闭环和质量门禁，不表示所有阶段必须机械串行。需求说明书完成并通过检查点后，可以在不破坏上下游契约的前提下并行推进独立产物。

进入需求分析前先按需求分级（XS / S / M-L）确定 loop 深度：小改动可裁剪产物阶段，但需求澄清、架构影响评估、API 契约冻结和容器化兼容检查等质量门禁在任何档位都不可裁剪。分级与裁剪规则详见根目录 `AGENTS.md`。

```text
需求分析
  -> 架构影响评估 + API 契约草案 + 容器化兼容检查
  -> 并行分支 A：功能测试用例
  -> 并行分支 B：UI 原型第一版
  -> 汇合门禁：测试用例完成后校准 UI 原型，冻结 API 契约
  -> 后端接口测试先行
  -> DRF 后端开发
  -> 前端 Playwright 测试先行
  -> Vue 3 前端开发
...
| 后端开发 | `back-end/` | DRF 代码、pytest 接口测试、迁移、pytest 运行输出与覆盖率证据 |
| 前端开发 | `front-end/` | Vue 代码、Playwright UI 测试、组件测试、Playwright 运行结果与关键截图证据 |
...
| 检查点 | 完成时机 | 必须确认 |
| --- | --- | --- |
| 需求澄清冻结 | 需求说明书冻结前 | 欠明确、有歧义或多解读的点是否已逐条澄清并闭环 |
| 架构影响评估 | 需求说明书阶段 | 模块边界、数据模型、权限、Jenkins、`api-test`、报告协议、Docker 部署是否受影响 |
| API 契约冻结 | 后端开发前 | 路径、方法、请求参数、响应字段、错误码、分页筛选、权限和状态流转 |
| UI 覆盖校准 | UI 原型第一版和功能测试用例完成后 | 原型是否覆盖正常、异常、边界、权限、加载、空状态、错误态和关键操作反馈 |
| 容器化兼容检查 | 需求、开发和联调验收阶段 | 是否存在本机路径、宿主机端口、真实凭据、手工 Jenkins 配置或不可迁移报告路径 |

subagent 协作原则：

- 需求说明书完成前不并行拆分下游开发，避免多个 agent 基于不同理解产出冲突设计。
- 需求说明书完成后，可以让测试设计 subagent 编写功能测试用例，同时让 UI 设计 subagent 输出第一版 UI 原型。
- 后端开发 subagent 必须等待功能测试用例和 API 契约冻结；前端开发 subagent 必须等待 UI 原型、功能测试用例和 API 契约都完成。
- Jenkins、Docker、`api-test` 执行器等基础设施任务可以与业务页面开发并行，但前提是需求已明确其接口、产物路径和验证方式。
- 主 agent 负责统一命名、分配文件、处理冲突、执行最终一致性审查；不得让多个 subagent 同时编辑同一个文件或同一份契约。
...
project-info/project_detail/execution-flow.drawio
project-info/project_detail/execution-flow.png
project-info/project_detail/execution-flow.drawio.png
```
...
& $drawio -x -f png --width 2200 -o "project-info\project_detail\execution-flow.png" "project-info\project_detail\execution-flow.drawio"
& $drawio -x -f png -e -s 2 -o "project-info\project_detail\execution-flow.drawio.png" "project-info\project_detail\execution-flow.drawio"
python C:\Users\admin\.codex\skills\drawio-skill\scripts\repair_png.py project-info\project_detail\execution-flow.drawio.png
```
