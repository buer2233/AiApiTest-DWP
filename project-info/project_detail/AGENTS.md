...
- 当前会话可用且任务需要可编辑架构图、流程图或 ER 图时，可选用 `drawio-skill`；Markdown 仍是架构语义真源。
- 当前会话可用且任务确实需要 PNG 等位图说明资产时，可选用 `imagegen`；位图不作为可编辑架构源文件。
- Skill 不可用时根据产物要求使用仓库现有工具链，不降低 XML 校验、导出质量、说明完整性或安全要求。
