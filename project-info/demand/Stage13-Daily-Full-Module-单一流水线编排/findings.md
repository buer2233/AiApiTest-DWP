...
- v2.2 实现前调研确认：当前 `init_admin` 会先校验变量并按同名账号提权/更新，违反 AC5.4。`--bootstrap-only` 必须先检查任意账号是否存在；存在时不得读取变量或修改账号。
...
- 主人于 2026-07-20 选择 C01 作为 Stage13 正式前端视觉基准，并确认当前受控浏览器渲染 PNG 可替代不可用的 `imagegen` 原始产物。前端采用“顶部快照条带 + 下方全宽环境台账”，并继续遵循 R1-R4 语义映射；C02-C05 的固定双栏、环境行、同步优先带和检查器布局不进入实现。
