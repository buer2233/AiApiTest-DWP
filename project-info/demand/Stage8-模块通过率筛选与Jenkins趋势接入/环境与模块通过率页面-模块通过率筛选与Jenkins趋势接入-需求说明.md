...
- **目标**：
  ...
  - 同一模块存在 `queued/running/canceling` 的失败重试或模块重试任务时，由后端统一拦截新的重试请求并提示 `已经有正在的本模块用例`；前端按钮不做该场景的禁用拦截。
  - Jenkins 任务弹窗支持取消任务、查看报告、查看 Jenkins 任务；外链由后端可信 Jenkins base URL 返回或派生，点击后在新标签页打开。
...
- **能做什么 / 做到什么程度 / 满足什么要求**：
  ...
  - 同一环境、同一模块已有 `queued/running/canceling` 且类型为 `failed_rerun/module_rerun` 的 Jenkins task 时，后端返回 `409 module_execution_locked`，错误文案固定为 `已经有正在的本模块用例`。
  - 前端在该后端互斥场景不提前禁用“一键失败重试”或“模块重试”按钮，点击后展示后端返回的业务提示。
...
| 2026-07-07 | v1.0.5 | 回写第三轮验收反馈：同模块活跃 Jenkins 重试任务由后端兜底互斥并返回 `已经有正在的本模块用例`；前端按钮保持可点击；取消任务对 Jenkins 404 转 409 业务冲突；外链新标签打开；真实验收允许触发 202 或后端互斥 409 | 主人指出排队任务过多、取消任务报错、外链无动作和按钮被前端禁用问题 |
