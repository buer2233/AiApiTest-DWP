...
| 本阶段状态 | 候选图已产出，等待主人选择；未进入 Figma 正式稿 |
...
校准结论：测试用例没有引入新的 UI 区域、字段或权限边界；R1-R4 映射可作为前端 TDD 输入。候选视觉与 Figma 正式稿仍须由主人选择后才能启动 Vue 编码。

## 8. 前端交付前门禁

- 主人必须在 C01-C05 中选择一个候选，并确认“浏览器渲染候选图可替代当前不可用的 imagegen 产物”后，才能进入 Figma 正式稿。
- Figma 正式稿须以选择的候选为视觉基准，补齐 R3/R4、member 权限态、加载/空/错误/冲突态和 375/768/1024/1440px frame；不得把复合状态画到 `/environments` 初始屏。
