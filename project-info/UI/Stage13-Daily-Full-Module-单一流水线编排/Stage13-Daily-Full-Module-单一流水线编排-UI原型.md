...
| 项 | 内容 |
| --- | --- |
| 需求状态 | 已冻结，依据需求说明 v2.2（2026-07-20） |
...
| 输入来源 | `project-info/demand/Stage13-Daily-Full-Module-单一流水线编排/Stage13-Daily-Full-Module-单一流水线编排-需求说明.md` 的 §3、§7、§8、§11、AC2.1-AC5.8；功能测试用例完成后须按同名 Stage13 用例补做覆盖校准 |
...
F5 仅改变固定 Jenkins Platform Bootstrap Job、Compose 一次性容器和后端管理命令，不新增浏览器路由、DRF API、页面字段、弹窗或前端状态。`/environments` 的 C01 与 R1-R4 映射保持不变；初始化成功或失败只在 Jenkins Summary、构建 Artifact 和既有部署诊断中可追溯，不得作为环境页面的新管理控件或敏感信息展示。
...
| 2026-07-20 | 1.1 | 按 v2.2 的 AC5 完成覆盖校准：自动 schema/首次数据初始化无新增前端范围，C01 与 R1-R4 不变。 |
